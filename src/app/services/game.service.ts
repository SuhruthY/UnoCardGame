...
import { GameState, GamePhase, PenaltyOption } from '../models/game.model';

@Injectable({ providedIn: 'root' })
export class GameService {
  private penaltyPool: PenaltyOption[] = [
  ...
  initGame(config: { name: string; isHuman: boolean }[] | { name: string; isHuman: boolean }[], adultMode?: boolean, storedPenalties?: Record<number, Record<string, boolean>>): void {
    const playersArr = Array.isArray(config) ? config : config as { name: string; isHuman: boolean }[];
    const isAdultMode = adultMode === true;

    this.nextId = 0;
    const deck = this.shuffle(this.createDeck());
    const playerList: Player[] = playersArr.map((p, i) => {
      const base: Player = {
        id: i,
        name: p.name,
        hand: [],
  ...
  playCard(cardId: number, chosenColor?: CardColor): void {
    ...
    let skippedPlayer: Player | null = null;
    ...
    this.state.update(st => ({
      ...
      players: st.players.map(p => {
        ...
        if (skippedPlayer && p.id === skippedPlayer.id) return { ...p, skipNextTurn: false };
        return p;
      }),
      drawnCards: newDrawn,
      discardPile: newDiscard,
  ...
  private aiAnalysis: { humanIdx: number; humanColors: Record<string, number>; humanNumbers: Record<number, number> } | null = null;
  ...
  private aiAnalyzeHuman(): void {
    const s = this.state();
    const humanIdx = s.players.findIndex(p => p.isHuman);
    if (humanIdx < 0) { this.aiAnalysis = null; return; }
    const human = s.players[humanIdx];
    const colors: Record<string, number> = { red: 0, blue: 0, green: 0, yellow: 0 };
    const numbers: Record<number, number> = {};
    for (const c of human.hand) {
      if (c.color !== 'wild') colors[c.color] = (colors[c.color] || 0) + 1;
      if (c.type === 'number' && c.value !== undefined) numbers[c.value] = (numbers[c.value] || 0) + 1;
    }
  ...
  private pickColorHumanFavors(): CardColor {
    if (this.aiAnalysis) {
      const colors: CardColor[] = ['red', 'blue', 'green', 'yellow'];
      const weighted = colors
        .map(c => ({ color: c, count: this.aiAnalysis!.humanColors[c] || 0 }))
        .sort((a, b) => b.count - a.count);
      if (weighted[0].count > 0) return weighted[0].color;
    }
  ...
  skipCurrentTurn(): void {
    this.state.update(s => ({
      ...
      players: s.players.map((p, i) =>
        i === s.currentPlayerIndex ? { ...p, skipNextTurn: false } : p
      ),
    }));
    this.endTurn();
  }

  selectPenalty(penaltyId: string): void {
    const s = this.state();
    if (s.phase !== 'penalty' || !s.penalty.waitingForSelection) return;
  ...
  applyPenalty(): void {
    const s = this.state();
    if (s.phase !== 'penalty' || s.penalty.waitingForSelection || !s.penalty.currentPenalty) return;
    ...
    switch (penaltyId) {
      ...
      case 'skips-ahead': {
        players = players.map(p =>
          p.id === loserId ? { ...p, skipNextTurn: true } : p
        );
      ...
      case 'wild-roulette': {
        players = players.map(p =>
          p.id === loserId ? { ...p, wildRoulette: true } : p
        );
      ...
      case 'hand-reveal': {
        players = players.map(p =>
          p.id === loserId ? { ...p, handRevealTurns: 2 } : p
        );
