...
describe('GameService', () => {
  ...
  it('deck has 108 cards', () => {
    const deck = (service as any).createDeck();
    expect(deck.length).toBe(108);
  });

  it('deck has correct card distribution', () => {
    const deck = (service as any).createDeck();
    const byColor = (color: string) => deck.filter((c: any) => c.color === color);
    const byType = (type: string) => deck.filter((c: any) => c.type === type);
  ...
  it('canPlayCard matches by color', () => {
    ...
    const red5 = { id: 999, color: 'red' as const, type: 'number' as const, value: 5 };
    (service as any).state.update((st: any) => ({ ...st, discardPile: [red5] }));
  ...
  it('canPlayCard matches by action type', () => {
    service.initGame([{ name: 'You', isHuman: true }, { name: 'Bot', isHuman: false }]);
    const skipCard = { id: 999, color: 'red' as const, type: 'skip' as const };
    (service as any).state.update((st: any) => ({ ...st, discardPile: [skipCard] }));
    ...
    expect(service.canPlayCard({ id: 3, color: 'red', type: 'number', value: 5 })).toBeTrue();
    expect(service.canPlayCard({ id: 4, color: 'blue', type: 'reverse' })).toBeFalse();
    expect(service.canPlayCard({ id: 5, color: 'wild', type: 'wild' })).toBeTrue();
  });
  ...
  it('canPlayCard with wild4 checks color restriction', () => {
    ...
    (service as any).state.update((st: any) => ({ ...st, players: [...st.players] }));

    expect(service.canPlayCard({ id: 1, color: 'wild', type: 'wild4' })).toBeFalse();

    // Remove all red cards from hand
    player.hand = player.hand.filter((c: any) => c.color !== 'red');
    (service as any).state.update((st: any) => ({ ...st, players: [...st.players] }));
  ...
  it('playCard removes card from hand', () => {
    ...
    const playable = player.hand.find((c: any) => service.canPlayCard(c));
    if (!playable) {
  ...
  it('skip card advances turn by 2 positions (4 players)', () => {
    ...
    const skipCard = { id: 5000, color: 'red' as const, type: 'skip' as const };
    player.hand.push(skipCard);
    const redAny = { id: 9999, color: 'red' as const, type: 'number' as const, value: 1 };
    (service as any).state.update((st: any) => ({
      ...st,
      players: [...st.players],
      discardPile: [redAny],
    }));
  ...
  it('reverse in 2-player acts as skip (same player goes again)', () => {
    service.initGame([{ name: 'You', isHuman: true }, { name: 'Bot', isHuman: false }]);
    const player = service.gameState().players[0];
    const revCard = { id: 5001, color: 'red' as const, type: 'reverse' as const };
  ...
  it('draw2 adds 2 to pending draw count (stacking)', () => {
    service.initGame([{ name: 'You', isHuman: true }, { name: 'Bot', isHuman: false }]);
    const player = service.gameState().players[0];
    const draw2 = { id: 5002, color: 'red' as const, type: 'draw2' as const };
  ...
  it('draw2 is playable on draw2 (stacking) when pending draw', () => {
    ...
    const draw2onDiscard = { id: 6000, color: 'red' as const, type: 'draw2' as const };
    const anotherDraw2 = { id: 6001, color: 'blue' as const, type: 'draw2' as const };
  ...
  it('mustDraw returns true when no playable cards', () => {
    ...
    const green5 = { id: 9999, color: 'green' as const, type: 'number' as const, value: 5 };
    (service as any).state.update((st: any) => ({
      ...st,
      players: [...st.players],
      discardPile: [green5],
  ...
  it('pickHumanColor returns a valid color', () => {
    const color = (service as any).pickHumanColor();
  ...
  it('game finishes when player runs out of cards', () => {
    ...
    const redDiscard = { id: 9999, color: 'red' as const, type: 'number' as const, value: 5 };
  ...
  it('initGame with stored penalties applies them to players', () => {
    const penalties: Record<number, Record<string, boolean>> = {
      1: { 'skips-ahead': true, 'wild-roulette': true },
    };
    service.initGame(
      [{ name: 'You', isHuman: true }, { name: 'Bot', isHuman: false }],
      false,
      penalties
    );
    const state = service.gameState();
  ...
  it('nextPenalty advances to next loser', () => {
    service.initGame(
      [{ name: 'You', isHuman: true }, { name: 'Bot', isHuman: false }, { name: 'C', isHuman: false }],
      true
    );
    const player = service.gameState().players[0];
    player.hand = [{ id: 10000, color: 'red', type: 'number', value: 5 }];
  ...
  it('skipCurrentTurn clears flag and ends turn', () => {
    ...
    (service as any).state.update((st: any) => ({
      ...st,
      players: st.players.map((p: any) => p.id === 0 ? { ...p, skipNextTurn: true } : p),
    }));
  ...
  it('handRevealTurns decrements on endTurn', () => {
    ...
    (service as any).state.update((st: any) => ({
      ...
      players: st.players.map((p: any) =>
        p.id === 0 ? { ...p, handRevealTurns: 2 } : p
      ),
    }));
    (service as any).endTurn();
    const state = service.gameState();
